			...
			function gotoPage(startIndex){
				...
				$.get(url, function(comments){
					if(comments && comments.dataSet && comments.dataSet.length > 0){
						...
						if(totalPage > 1){
							if(currentPage == 1){
								page += '<li class="linkPage" style=" color:#CCCCCC">上一页</li><li class="linkPage" onclick="nextPage()">下一页</li>';
							}else if(currentPage == totalPage){
								page += '<li class="linkPage" onclick="prevPage()">上一页</li><li class="linkPage" style=" color:#CCCCCC">下一页</li>';
							}else{
						...
						for(var i=0; i<length; i++){
							...
							var userText = comment.reply=='' 
								...
								?('<span style="color:#492bff;" >'+ comment.name+'</span>'+ '：发表于 '+comment.dateTime+ '<br />' + comment.text )
								: ('<span style="color:#492bff;" >'+comment.name+'</span>' + '：发表于 '+comment.dateTime+ '<br />' + comment.text + '<br /><br />' + comment.reply);
							var html = '<ul class="userDiscuss">' + 
								...
								'<li class="userTime">'+ 
								'<div style="top:0px;right:15px;position: absolute;color:#492bff;font-weight:bold;cursor:pointer;" onclick="showReply(this)">回复</div>' +
								'</li>' +
								...
								'<input name="reply" type="button" value="提交" style="cursor:pointer;border:0px solid #65b74c;background:none;width:50px;height:30px;color:#492bff;font-weight:bold;"/>&nbsp;' +
								'<input name="cancel" type="button" value="取消" style="cursor:pointer;border:0px solid #65b74c;background:none;width:50px;height:30px;color:#492bff;font-weight:bold;"/>' +
								'</div></form></li>' +
								'</ul>';
				    	...
				    	<li class="discussMiddle">
							<ul class="discussInputBox">
								<li>
									...
									<textarea name="comment" id="discussContentInfoBox"  style="width:665px;height:105px;line-height:18px;"></textarea>
									<div style="width:670px;padding:5px 0;text-align:left;position:relative;">
										请输入150字以内的回复
										...
										<input id="publish" type="button" value="" class="commentBT" />
									</div>
